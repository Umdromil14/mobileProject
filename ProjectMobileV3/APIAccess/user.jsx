...
async function login(
    ...
    console.log("avant 2eme if");
    if (isValid) {
        try {
            console.log("avant post login");
            ...
            console.log("avant set token");
            await AsyncStorage.setItem("token", response.data.token);
            console.log("après set token");
            //})
        } catch (reason) {
            throw reason;
        //.catch((reason) => {
...
async function updateUser(username, firstName, lastName, email) {
    ...
    try {
        const response = await axiosInstance.patch(APIURL + "/user/",
            updateValues,
            { headers: { Authorization: await getAuthorizationHeader() } });
