...
function FlatListVideoGame({ platform, publicationsData }) {
    ...
    return (
        <FlatList
            ...
            renderItem={({ item }) =>
                <Pressable
                    style={{ marginLeft: 14, borderWidth: 1, borderColor: GREEN }}
                    onPress={() =>
                        navigation.navigate("GamePreview",
                            { videoGameId: item.video_game_id, actualPlatform: item.platform_code })
                    }>
                    <Image style={{ height: 150, width: 100 }} source={images[item.video_game_id]} />
                </Pressable>
            }
            horizontal={true}
...
function FlatListPlatform() {
    ...
    useEffect(() => {
        getPublications(undefined, undefined, undefined, undefined, true).then((response) => {
            setMyGames(response);
        }).catch((reason) => {
        ...
        Promise.all([
            getPlatforms(),
            getPublications(undefined, undefined, undefined, undefined, false, true)
        ])
            .then((response) => {
